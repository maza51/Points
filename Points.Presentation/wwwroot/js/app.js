var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
import { pointsApi } from "./api/pointApi.js";
import { KonvaPoint } from "./konva/konvaPoint.js";
import { commentsApi } from "./api/commentApi.js";
var App = /** @class */ (function () {
    function App() {
        this.stage = null;
        this.layer = null;
        this.points = [];
    }
    App.prototype.init = function () {
        return __awaiter(this, void 0, void 0, function () {
            var _a;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        _a = this;
                        return [4 /*yield*/, pointsApi.getPoints()];
                    case 1:
                        _a.points = _b.sent();
                        this.setupForm();
                        this.setupKonva();
                        this.renderPoints();
                        this.setupEventListeners();
                        return [2 /*return*/];
                }
            });
        });
    };
    App.prototype.setupForm = function () {
        var pointSelect = document.getElementById("point-select");
        while (pointSelect.firstChild) {
            pointSelect.removeChild(pointSelect.firstChild);
        }
        for (var _i = 0, _a = this.points; _i < _a.length; _i++) {
            var point = _a[_i];
            console.log(point);
            var newOption = document.createElement("option");
            newOption.value = point.id.toString();
            newOption.text = point.id.toString();
            pointSelect.add(newOption);
            console.log(newOption);
        }
    };
    App.prototype.setupKonva = function () {
        this.stage = new Konva.Stage({
            container: "stage-container",
            width: 930,
            height: 930,
        });
        this.layer = new Konva.Layer();
        this.stage.add(this.layer);
    };
    App.prototype.renderPoints = function () {
        for (var _i = 0, _a = this.points; _i < _a.length; _i++) {
            var point = _a[_i];
            this.renderPoint(point);
        }
    };
    App.prototype.renderPoint = function (point) {
        var konvaPoint = new KonvaPoint(point);
        this.layer.add(konvaPoint.getElement());
    };
    App.prototype.setupEventListeners = function () {
        var _this = this;
        var addButton = document.getElementById("add-button");
        addButton.addEventListener("click", function () { return __awaiter(_this, void 0, void 0, function () {
            var createdPoint, error_1;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        return [4 /*yield*/, pointsApi.createPoint()];
                    case 1:
                        createdPoint = _a.sent();
                        this.points.push(createdPoint);
                        this.renderPoint(createdPoint);
                        this.setupForm();
                        return [3 /*break*/, 3];
                    case 2:
                        error_1 = _a.sent();
                        console.log(error_1);
                        return [3 /*break*/, 3];
                    case 3: return [2 /*return*/];
                }
            });
        }); });
        var addCommentButton = document.getElementById("add-comment-button");
        addCommentButton.addEventListener("click", function () { return __awaiter(_this, void 0, void 0, function () {
            var pointSelect, commentInput, colorSelect, comment, createdComment, error_2;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        pointSelect = document.getElementById("point-select");
                        commentInput = document.getElementById("comment-input");
                        colorSelect = document.getElementById("color-select");
                        comment = {
                            text: commentInput.value,
                            backgroundColor: colorSelect.value,
                            pointId: parseInt(pointSelect.value)
                        };
                        _a.label = 1;
                    case 1:
                        _a.trys.push([1, 3, , 4]);
                        return [4 /*yield*/, commentsApi.createComment(comment)];
                    case 2:
                        createdComment = _a.sent();
                        location.reload();
                        return [3 /*break*/, 4];
                    case 3:
                        error_2 = _a.sent();
                        console.log(error_2);
                        return [3 /*break*/, 4];
                    case 4: return [2 /*return*/];
                }
            });
        }); });
    };
    return App;
}());
export { App };
var app = new App();
app.init();
//# sourceMappingURL=app.js.map